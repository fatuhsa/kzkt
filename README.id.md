...
| UI | Jetpack Compose (BOM 2026.01.01), Material 3 1.5.0-alpha25, MaterialKolor (tema Material You dinamis), Navigation Compose 2.8.5, Coil 2.7 |
| Konkurensi & State | Coroutines 1.9, Flow, ViewModel, DataStore Preferences 1.1.2 |
| Machine Learning | ONNX Runtime 1.21 (YOLO), ML Kit Text Recognition 16.0.1 (Latin + Jepang) |
| Computer Vision | OpenCV 4.10.0 Android SDK (`libopencv_java4.so` C++ JNI) |
| Networking & JSON | OkHttp 4.12, Gson 2.11 |
| Persistensi & Background | DataStore Preferences, WorkManager 2.10, MediaStore |
...
├── .github/workflows/          # Android CI + Auto Release (APK per-ABI)
├── build.gradle.kts
├── settings.gradle.kts
...
- **Android CI** (`android.yml`) — setiap push / pull request ke `main`, unit test dan
  `assembleDebug` dijalankan di image Docker yang sama, lalu APK debug-nya diunggah sebagai
  **artifact** yang bisa diunduh (`kzkt-app-debug`).
- **Auto Release** (`kzkt.yml`) — build **APK release signed** untuk empat ABI
